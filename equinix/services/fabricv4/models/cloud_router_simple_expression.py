...
class CloudRouterSimpleExpression(BaseModel):
    ...
    """ # noqa: E501
    var_property: Optional[StrictStr] = Field(default=None, description="Possible field names to use on filters:  * `/project/projectId` - project id (mandatory)  * `/name` - Fabric Cloud Router name  * `/uuid` - Fabric Cloud Router uuid  * `/state` - Fabric Cloud Router status  * `/location/metroCode` - Fabric Cloud Router metro code  * `/location/metroName` - Fabric Cloud Router metro name  * `/package/code` - Fabric Cloud Router package  * `/type` - Fabric Cloud Router type  * `/connectionsCount` - Fabric Cloud Router attached connections count  * `/changeLog/createdDateTime` - Fabric Cloud Router creation timestamp  * `/changeLog/updatedDateTime` - Fabric Cloud Router last updated timestamp  * `/changeLog/deletedDateTime` - Fabric Cloud Router deletion timestamp  * `/*` - all-category search ", alias="property")
