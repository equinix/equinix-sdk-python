...
class MetricSimpleExpression(BaseModel):
    ...
    """ # noqa: E501
    var_property: Optional[StrictStr] = Field(default=None, description="Possible field names to use on filters:  * `/subject` - Metric subject description (required and limited to 1 value)  * `/name` - Metric names (required)  * `/dataPoints/endDateTime` - Time of Metrics  * `/time` - Time of Metrics  * `/interval` - ISO-8601 duration format to specify the interval of metrics data points  * `/value` - specific data points of the metric to return (e.g. 'last' - for metros only) ", alias="property")
